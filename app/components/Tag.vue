...
const resolvedTag = computed(() => ({
  id: props.id ?? props.tag?.id ?? "",
  title: props.title ?? props.tag?.title ?? "",
  isInactive: props.isInactive ?? props.tag?.isInactive,
  hidden: props.hidden ?? props.tag?.hidden ?? false,
  associatedIcon: props.associatedIcon ?? props.tag?.associatedIcon,
}));

const resolvedTitle = computed(() => resolvedTag.value.title || "nos projets");
</script>

<template>
  <Button
    class="tag"
    @click="emit('select', resolvedTag.id, resolvedTitle)"
    :title="`Découvrir nos vidéos labelisées '${resolvedTitle}'`"
    :disableArrow="true"
